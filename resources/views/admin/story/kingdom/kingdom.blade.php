...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" >
    <div class="modal-content">
        ...
        <form action="{{ url('admin/story/kingdom') }}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label for="kingdom_title" class="col-form-label">title</label>
                        <input type="text" name="kingdom_title" class="form-control" id="title">
    ...
    <div class="card">
        <div class="card-header" style="background-color: #D3D3D3">
            <nav class="navbar navbar-expand-lg navbar-dark">
                <a class="navbar-brand" style="color: black; margin-left:5px" href="#">Kingdom</a>


                <form class="form-inline my-2 my-lg-0 d-flex justify-content-between w-75" method="GET" action="{{ route('admin.story.kingdom.kingdom') }}">
    ...
    @endif
        <div class="card-body" style="background-color: #D3D3D3">
            <table class="table table-bordered table-striped" style="background-color: white; border-spacing: 10px; border-radius: 30px;">
                ...
                @foreach ($kingdom as $key => $kingdomItem)
                <tr>
                    <td><strong>{{ $kingdomItem->kingdom_title }}</strong></td>
                    <td style="display: flex; align-items: center;">
                        @if($key < 4)
                            @if($key == 0)
                                <a href="{{ url('admin/story/kingdom/kingdom_home/kingdom_home') }}" class="btn btn-sm btn-success" style="background-color: #194B46; color: white">Edit</a>
                            @elseif($key == 1)
                                <a href="{{ url('admin/story/kingdom/gallery/gallery') }}" class="btn btn-sm btn-success" style="background-color: #194B46; color: white">Edit</a>
                            @endif
                        @else
                            <a href="" class="btn btn-sm btn-success" style="background-color: #194B46; color: white">Edit</a>
                        @endif
                        <form action="{{ route('story.kingdom.destroy', $kingdomItem->kingdom_id) }}" method="post" onsubmit="return confirm('Are you sure you want to delete this story?');">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
