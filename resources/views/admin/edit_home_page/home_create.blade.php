...
<div class="row">
    <div class='col-md-12'>
        <div class="card" style="background-color: #D3D3D3">
            ...
            <div class="card-body">
                <form action="{{ url('admin/edit_home_page') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label>Title</label>
                            <textarea type="text" name="home_title" class="form-control"></textarea>
                            @error('home_title') <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Sub_title</label>
                            <textarea name="home_subtitle" id="textarea" class="form-control"></textarea> <!-- Change input to textarea -->
                            @error('home_subtitle') <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="home_image">Upload Background Image</label>
                            <div class="upload-box d-flex justify-content-center align-items-center">
                              <input type="file" id="home_image" name="home_image" class="form-control d-none" accept=".jpg,.jpeg,.png">
